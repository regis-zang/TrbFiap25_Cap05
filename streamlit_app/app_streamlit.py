...
st.set_page_config(page_title="Dashboard Vendas (Preview)", layout="wide")
...
with tab1:
    ...
    # Série temporal
    s = (df_f.dropna(subset=["_data_pedido"]).sort_values("_data_pedido")
         .groupby("mes").agg(Receita=("receita","sum"),
                             Pedidos=("pedido_id","nunique"),
                             Itens=("itens","sum")).reset_index())
    ...
    if "categoria" in df_f.columns and not df_f["categoria"].dropna().empty:
        g = (df_f.groupby("categoria", dropna=False)["receita"].sum().sort_values(ascending=False).head(15)).reset_index()
    ...
    if "canal" in df_f.columns and not df_f["canal"].dropna().empty:
        g = (df_f.groupby("canal", dropna=False)["receita"].sum().sort_values(ascending=False)).reset_index()
    ...
    if "responsavelpedido" in df_f.columns and not df_f["responsavelpedido"].dropna().empty:
        g = (df_f.groupby("responsavelpedido", dropna=False)["receita"].sum()
